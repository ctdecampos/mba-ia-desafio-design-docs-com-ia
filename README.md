...
4. **Fase do FDD (Detalhamento Técnico):** Descemos ao nível de baixo nível de código, detalhando payloads, rotas, matriz de erros com prefixo `WEBHOOK_`, tratamento de retries e a seção crítica de integração com arquivos reais do boilerplate (`order.service.ts`, `requireRole.ts`, `errorHandler.ts`).
...
7. **Refatoração do README e Empacotamento:** Finalizamos documentando nosso processo neste README e criando um script automático de compilação de artefatos.
...
3. Verifique se o código existente possui referências a: 'requireRole', 'errorHandler', 'order.service.ts' e 'AppError'.
...
### Prompt 2: Engenharia de Baixo Nível para o FDD (Resiliência e Contratos)
```markdown
...
- A seção obrigatória 'Integração com o sistema existente' referenciando caminhos de arquivos reais e trechos de pseudocódigo TypeScript.
```

---

## 5. Iterações e Ajustes
A qualidade e densidade da documentação exigiram 4 ciclos completos de iterações e correções críticas:
...
* **Ajuste 2 (Filtro de Escopo e Alucinações):** A IA sugeriu de forma autônoma a inclusão de um sistema de alertas de webhook por e-mail e uma interface administrativa visual no PRD. Tivemos que repreendê-la e solicitar a remoção, mantendo o foco estrito no que foi decidido em reunião: e-mail e painel visual foram expressamente marcados como fora de escopo para esta versão.
* **Ajuste 3 (Padronização de Erros):** No primeiro esboço do FDD, a IA usou códigos genéricos de erro como `INVALID_URL` ou `NOT_FOUND`. Ajustamos o prompt para forçar a aderência aos padrões da nossa codebase, gerando os códigos específicos com o prefixo corporativo `WEBHOOK_*` (ex: `WEBHOOK_INVALID_URL`).
* **Ajuste 4 (Validação do Tracker):** Corrigimos o mapeamento de localização do Tracker, que inicialmente continha referências vagas como "Metade da call", exigindo a indicação exata dos timestamps lidos (ex: `[09:18] Diego`) e caminhos de arquivo reais (`src/middlewares/requireRole.ts`).
...
4. **Detalhamento de Implementação:** [docs/FDD.md](./docs/FDD.md) - Especificação matemática de resiliência, contratos JSON e integração com o código existente.
